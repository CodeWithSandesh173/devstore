...
function displayProducts(products) {
    ...
    products.forEach(product => {
        const lowestPrice = product.packages[0].price;
        let usdPrice;

        if (product.currency === 'USD') {
            usdPrice = parseFloat(lowestPrice).toFixed(2);
        } else {
            usdPrice = (lowestPrice / 145).toFixed(2);
        }
...
function buyNow(productId) {
    ...
    product.packages.forEach((pkg, index) => {
        let usdPrice;
        if (product.currency === 'USD') {
            usdPrice = parseFloat(pkg.price).toFixed(2);
        } else {
            usdPrice = (pkg.price / 145).toFixed(2);
        }
        ...
        packagesHTML += `
            <div class="card package-option" data-product-id="${product.id}" data-package-index="${index}" style="margin-bottom: 0.5rem; cursor: pointer;">
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    ...
                    <span class="text-primary">$${usdPrice}</span>
                </div>
            </div>
        `;
    });
